...
namespace {
    ...
    void accept_iwire_data
        ( ::b64_iwire * wire, const char * data, size_t size )
    {
        std::ostream& stream = *static_cast<std::ostream*>(wire->object);
        stream.write(data, size);
    }

    void run ( std::istream& stream, ::b64_iwire& wire )
    {
        char data[1024];
        std::size_t size = 0;
        std::size_t pass = 0;
        ...
        do {
            ...
            pass = stream.gcount();
            size += pass;
            used = ::b64_iwire_feed(&wire, data, size);
            std::copy(data+used, data+size, data);
            size -= used;
        }
        while (pass > 0);
        ::b64_iwire_done(&wire, data, size);
    }
